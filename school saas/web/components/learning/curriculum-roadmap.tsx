        ...
        <div className="bg-slate-950/40 backdrop-blur-md border border-border rounded-3xl p-6 shadow-2xl animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-foreground uppercase tracking-tight">NERDC Roadmap</h2>
                        <p className="text-muted-foreground text-xs">Current Academic Session</p>
                    </div>
                </div>
                <div className="text-right">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">Current Velocity</p>
                ...
                <div className="space-y-6 relative z-10">
                    {items.length > 0 ? (
                        items.map((item, idx) => (
                            <div key={item.id} className="flex gap-4 group">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 border-4 transition-all ${item.status === 'completed' ? 'bg-emerald-500 border-slate-900 text-black shadow-[0_0_15px_rgba(16,185,129,0.3)]' :
                                    item.status === 'in-progress' ? 'bg-blue-600 border-slate-900 text-foreground shadow-[0_0_15px_rgba(37,99,235,0.3)] animate-pulse' :
                                ...
                                <div className={`flex-1 p-4 rounded-2xl border transition-all ${item.status === 'in-progress' ? 'bg-secondary/50 border-blue-500/30' :
                                    'bg-transparent border-transparent hover:bg-secondary/50 hover:border-border/50'
                                    }`}>
                                    <div className="flex justify-between items-start mb-1">
                                        <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">{item.week_range || item.week}</span>
                                        {item.status === 'in-progress' && (
                                            <span className="text-[9px] bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-full font-bold uppercase">Active</span>
                                        )}
                                    </div>
                                    <h3 className={`font-bold ${item.status === 'locked' ? 'text-muted-foreground' : 'text-foreground'}`}>
                        ...
                        <div className="text-center py-10 border border-dashed border-border rounded-2xl">
                            ...
                            <p className="text-muted-foreground text-sm">Curriculum data syncing...</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <button
                ...
                className="w-full mt-8 bg-secondary/50 hover:bg-white/10 text-slate-300 font-bold py-3 rounded-xl uppercase tracking-widest text-xs flex items-center justify-center gap-2 border border-border/50 transition-all"
            >
