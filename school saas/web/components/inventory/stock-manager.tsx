                ...
                <div className="flex gap-2">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button
                                variant="ghost"
                                className={`border border-border ${categoryFilter !== 'all' ? 'bg-slate-800 text-foreground border-white/20' : 'text-slate-300'} hover:bg-slate-800 hover:text-foreground transition-colors`}
                            >
                    ...
                    <Button className="bg-[var(--school-accent)] text-foreground" onClick={() => setIsAddOpen(true)}>
                            ...
                            <TableRow key={item.id} className="border-border/50 hover:bg-transparent">
                                ...
                                <TableCell className="text-center font-mono text-slate-300">
                                    <div className="flex flex-col items-center">
                                        <span className={`font-bold ${item.quantity_on_hand <= item.reorder_level ? 'text-red-500' : 'text-foreground'}`}>
                                            {item.quantity_on_hand}
                                        </span>
                                        <span className="text-[10px] text-muted-foreground uppercase">{item.unit_type}</span>
                                    </div>
                                </TableCell>
                                ...
                                <TableCell>
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-card text-card-foreground border-border">
                                            <DropdownMenuItem className="text-slate-300 focus:bg-slate-800 focus:text-foreground cursor-pointer">
                                                <Edit className="mr-2 h-4 w-4" /> Edit
                                            </DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => setRestockItem(item)} className="text-slate-300 focus:bg-slate-800 focus:text-foreground cursor-pointer">
                ...
                <AlertDialogContent className="bg-slate-950 border-border text-foreground">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-muted-foreground">
                            Are you sure you want to delete <span className="font-bold text-foreground">{deleteItem?.name}</span>?
