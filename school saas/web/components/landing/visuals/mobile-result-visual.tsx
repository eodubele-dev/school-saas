...
export function MobileResultVisual() {
    return (
        <div className="relative w-[280px] h-[580px] bg-slate-950 rounded-[3rem] border-[8px] border-slate-900 shadow-2xl overflow-hidden mx-auto">
            {/* Phone Notch/Speaker */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 h-6 w-32 bg-card text-card-foreground rounded-b-xl z-20"></div>

            {/* Screen Content */}
            <div className="w-full h-full bg-slate-50 relative pt-12 px-4 pb-8 flex flex-col">
                {/* School Header */}
                <div className="flex flex-col items-center mb-6">
                    <div className="h-12 w-12 bg-blue-900 rounded-full mb-2 flex items-center justify-center text-foreground font-bold">E</div>
                    <h3 className="text-slate-900 font-bold text-sm">EduCare High School</h3>
                    <p className="text-[10px] text-muted-foreground">Term 2 Result Sheet</p>
                </div>
                ...
                <div className="flex gap-3 mb-6 bg-white p-3 rounded-xl shadow-sm border border-slate-100">
                    ...
                    <div>
                        ...
                        <div className="text-[10px] text-muted-foreground">JSS 3 Blue</div>
                ...
                <div className="flex-1 relative mb-4">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="bg-card text-card-foreground/90 backdrop-blur-md rounded-2xl p-6 text-center w-[90%] shadow-xl transform scale-105 border border-border">
                            <div className="h-10 w-10 bg-red-500/20 text-red-500 rounded-full flex items-center justify-center mx-auto mb-3">
                                <Lock className="h-5 w-5" />
                            </div>
                            <h4 className="text-foreground font-bold text-sm mb-1">Result Locked</h4>
                            <p className="text-[10px] text-muted-foreground mb-4">
                                Please pay school fees to view full academic results.
                            </p>
                            <button className="w-full py-2.5 bg-green-500 hover:bg-green-600 text-foreground rounded-lg text-xs font-bold shadow-lg shadow-green-900/20 transition-transform hover:scale-105">
                                Pay ₦45,000 Now
                            </button>
                        </div>
                    </div>
