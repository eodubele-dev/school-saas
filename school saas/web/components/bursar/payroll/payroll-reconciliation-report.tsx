...
export const PayrollReconciliationReport: React.FC<PayrollReconciliationReportProps> = ({ staffData, month, year }) => {
    ...
    return (
        <div className="bg-[#0A0A0B] border border-border rounded-3xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                ...
                <button className="bg-secondary/50 border border-border text-foreground px-6 py-2.5 rounded-2xl flex items-center gap-2 hover:bg-white/10 transition-all active:scale-95 font-bold text-xs uppercase tracking-widest group">
                    <Download size={16} className="text-cyan-400 group-hover:animate-bounce" /> Export for Bank
                </button>
            </div>

            <div className="overflow-hidden rounded-2xl border border-border/50 bg-white/[0.01]">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-secondary/50 border-b border-border">
                            <th className="py-4 px-6 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Staff Member</th>
                            <th className="py-4 px-6 text-[10px] font-black text-muted-foreground uppercase tracking-widest text-center">Net Attendance</th>
                            <th className="py-4 px-6 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Forensic Flags</th>
                            <th className="py-4 px-6 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Reconciliation Status</th>
                    ...
                    <tbody className="text-sm">
                        {staffData.map((staff) => (
                            <React.Fragment key={staff.id}>
                                <tr
                                    className={cn(
                                        "border-b border-border/50 hover:bg-white/[0.03] transition-colors cursor-pointer group",
                                ...
                                >
                                    <td className="py-6 px-6">
                                        <div className="flex flex-col">
                                            <span className="font-bold text-slate-200 group-hover:text-foreground transition-colors uppercase tracking-tight">{staff.name}</span>
                                            <span className="text-[10px] text-muted-foreground font-mono uppercase mt-1">{staff.role}</span>
                                        </div>
                                    </td>
                                    <td className="py-6 px-6 text-center">
                                        <div className="flex flex-col items-center">
                                            <span className="text-foreground font-black text-lg">{staff.daysPresent}</span>
                                ...
                                {expandedRow === staff.id && staff.flags > 0 && (
                                    <tr className="bg-black/40 border-b border-border/50 animate-in slide-in-from-top-2 duration-300">
                                        <td colSpan={5} className="p-0">
                                            <div className="p-8 space-y-4">
                                                <div className="flex items-center gap-2 mb-4">
                                                    <AlertCircle size={16} className="text-amber-500" />
                                                    <h4 className="text-[10px] font-black text-foreground uppercase tracking-[0.2em]">Forensic Flag Detail: {staff.name}</h4>
                                                </div>

                                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                    {staff.overrideDetails.map((detail, idx) => (
                                                        <div key={idx} className="bg-secondary/50 border border-border/50 rounded-2xl p-5 space-y-3 relative overflow-hidden group/item">
                                                            ...
                                                            <div className="relative z-10">
                                                                ...
                                                                <div className="flex gap-3 items-start bg-black/40 p-3 rounded-xl border border-border/50">
