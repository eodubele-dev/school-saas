...
export function ExamInterface({ quiz, questions, onComplete }: { quiz: any, questions: any[], onComplete: (answers: Record<string, string>) => void }) {
    ...
    return (
        <div className="h-screen w-full bg-slate-950 flex flex-col overflow-hidden">
            ...
            <div className="h-16 border-b border-border flex items-center justify-between px-6 bg-card text-card-foreground">
                <div className="flex items-center gap-4">
                    <MonitorOff className="h-5 w-5 text-muted-foreground" />
                    <div>
                        <h1 className="text-foreground font-bold text-sm md:text-base line-clamp-1">{quiz.title}</h1>
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="hidden md:flex w-64 border-r border-border/50 bg-card text-card-foreground/50 flex-col p-4 overflow-y-auto">
                    <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">Navigator</h3>
                        ...
                        {questions.map((q, i) => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    currentIdx === i ? "ring-2 ring-white bg-slate-800 text-foreground" : "",
                                    answers[q.id]
                                        ? "bg-[var(--school-accent)] text-foreground shadow-[0_0_10px_rgba(59,130,246,0.3)]"
                                        : "bg-card text-card-foreground border border-border text-muted-foreground hover:text-foreground"
                                )}
                            >
                                {i + 1}
                            </button>
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            answers[currentQ.id] === optKey
                                                ? "bg-[var(--school-accent)]/10 border-[var(--school-accent)] text-foreground"
                                                : "bg-card text-card-foreground border-border text-slate-300 hover:bg-slate-800 hover:border-white/20"
                                        )}
                                    >
            ...
            {/* Footer Nav */}
            <div className="h-20 border-t border-border bg-card text-card-foreground px-6 flex items-center justify-between">
                <Button
                    variant="ghost"
                    disabled={currentIdx === 0}
                    onClick={() => setCurrentIdx(prev => prev - 1)}
                    className="bg-transparent text-muted-foreground hover:bg-secondary/50 hover:text-foreground"
                >
                ...
                {currentIdx < questions.length - 1 ? (
                    ...
                    <Button
                        ...
                        className="bg-green-600 hover:bg-green-700 text-foreground font-bold px-8 shadow-[0_0_20px_rgba(22,163,74,0.4)]"
                    >
                        Submit Exam
                    </Button>
                )}
            ...
            {confirmSubmit && (
                <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
                    <Card className="max-w-md w-full p-8 bg-card text-card-foreground border-border text-center">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Button
                                ...
                                className="bg-transparent border-border text-foreground hover:bg-secondary/50 hover:text-foreground"
                            >
                                Go Back
                            </Button>
                            <Button onClick={handleSubmit} className="bg-green-600 hover:bg-green-700 text-foreground">
                                Confirm Submit
                            </Button>
                        </div>
                    </Card>
