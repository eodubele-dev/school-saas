...
export function AuditItem({ log }: AuditItemProps) {
    ...
    return (
        <Card className={`
            ...
            ${isRedFlag ? 'border-red-500/30 shadow-[0_0_15px_-3px_rgba(239,68,68,0.2)]' : 'border-border'}
        `}>
            {/* Header Row */}
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                    <div className={`
                        ...
                        ${isRedFlag ? 'bg-red-500/10 border-red-500/20' : 'bg-card text-card-foreground border-border'}
                    `}>
                        {getIcon()}
                    </div>
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="text-sm font-mono text-slate-300">
                                <span className={isRedFlag ? 'text-red-400 font-bold' : 'text-foreground font-medium'}>
                                    {log.actor_name}
                                </span>
                            ...
                            <Badge variant="outline" className={`
                                ...
                                ${log.action === 'DELETE' ? 'text-red-400 bg-red-500/5' : 'text-muted-foreground bg-slate-800/50'}
                            `}>
                                {log.action}
                            </Badge>
                        </div>
                        <p className="text-muted-foreground text-sm mt-1 leading-relaxed max-w-2xl font-mono">
                            {log.details}
                        </p>

                        {/* Grade Change Visualization */}
                        {log.old_values && log.new_values && log.entity_type === 'grade' && (
                            <div className="mt-3 flex items-center gap-3 text-xs font-mono bg-slate-950/50 p-2 rounded border border-border w-fit">
