    ...
    return (
        <div className="bg-slate-950/40 backdrop-blur-md border border-border rounded-3xl p-8 shadow-2xl animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-2xl font-bold text-foreground uppercase tracking-tight italic">Authorized Pick-up Persons</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {data.length > 0 ? (
                    data.map((person) => (
                        <div key={person.id} className="bg-secondary/50 border border-border p-6 rounded-2xl flex items-center gap-4 relative group hover:bg-white/10 transition-colors">
                    ...
                    <div className="col-span-1 md:col-span-2 text-center py-12 border border-dashed border-border rounded-2xl">
                        <UserPlus className="mx-auto text-slate-700 mb-3" size={32} />
                        <p className="text-muted-foreground text-sm">No authorized persons added yet.</p>
                    </div>
                )}
            </div>
        </div>
