    ...
    return (
        <div className="flex flex-col lg:flex-row h-[700px] divide-y lg:divide-y-0 lg:divide-x divide-white/5">
            ...
            <div className="w-full lg:w-1/3 flex flex-col bg-black/20">
                <div className="p-4 border-b border-border/50 bg-card text-card-foreground/40 backdrop-blur-sm">
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                        <Input
                            placeholder="Scan QR or Enter Code..."
                            className="pl-10 bg-black border-border text-slate-200 focus:border-indigo-500/50 font-mono"
                            value={searchQuery}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {filteredPasses.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-muted-foreground text-sm italic py-10">
                        ...
                        filteredPasses.map(pass => (
                            <div
                                ...
                                className={`p-4 rounded-xl border transition-all cursor-pointer ${selectedPass?.id === pass.id ? 'bg-indigo-500/10 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.1)]' : 'bg-card text-card-foreground/50 border-border/50 hover:border-white/20'}`}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className="font-mono font-bold text-foreground text-sm">{pass.pass_code}</span>
                                </div>
                ...
                {selectedPass ? (
                    <div className="max-w-md mx-auto w-full animate-in zoom-in-95 duration-500">
                        <div className="text-center mb-8">
                            ...
                            <h2 className="text-3xl font-black text-foreground font-mono tracking-widest">{selectedPass.pass_code}</h2>
                            <p className="text-indigo-400 font-bold uppercase tracking-widest mt-2">{selectedPass.type.replace('_', ' ')}</p>
                        </div>

                        <div className="bg-card text-card-foreground/80 backdrop-blur-xl border border-border rounded-2xl p-6 mb-8 shadow-2xl space-y-4">
                                ...
                                <div className="flex items-center gap-4 pb-4 border-b border-border/50">
                                    <div className="w-12 h-12 rounded-full bg-slate-800 border-2 border-indigo-500/30 overflow-hidden flex items-center justify-center">
                                        {selectedPass.student.photo_url ? (
                                            ...
                                            <span className="text-[10px] text-muted-foreground font-mono">PHOTO</span>
                                        )}
                                    </div>
                                    <div>
                                        <p className="font-bold text-foreground text-lg">{selectedPass.student.full_name}</p>
                                        <p className="text-xs text-muted-foreground font-mono">{selectedPass.student.admission_number}</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex justify-between items-center pb-4 border-b border-border/50">
                                    <span className="text-sm text-muted-foreground">Target ID</span>
                                    <span className="font-mono text-sm text-foreground">{selectedPass.student_id}</span>
                                </div>
                            )}

                            <div className="flex justify-between items-center">
                                <span className="text-sm text-muted-foreground">Reason</span>
                                <span className="font-bold text-sm text-foreground max-w-[200px] text-right truncate">{selectedPass.reason}</span>
                            </div>
                        ...
                        {new Date(selectedPass.valid_until) < new Date() ? (
                            ...
                            <Button
                                ...
                                className="w-full bg-indigo-600 hover:bg-indigo-500 text-foreground shadow-[0_0_20px_rgba(79,70,229,0.4)] h-14 text-lg font-black uppercase tracking-widest gap-2 group"
                            >
