...
export function TransferModal({ isOpen, onClose, student, classes }: TransferModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="bg-slate-950 border-border text-foreground sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-muted-foreground">
                        Move <span className="text-foreground font-bold">{student.name}</span> to a new class.
                        This will update their current placement immediately.
                    </DialogDescription>
                </DialogHeader>

                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-2 items-center gap-4 p-3 rounded-lg bg-secondary/50 border border-border">
                        <div>
                            <p className="text-[10px] uppercase text-muted-foreground font-bold">Current Class</p>
                            <p className="font-mono text-sm">{student.class}</p>
                        </div>
                        <ArrowRight className="h-4 w-4 text-muted-foreground mx-auto" />
                        <div className="text-right">
                            <p className="text-[10px] uppercase text-muted-foreground font-bold">New Class</p>
                ...
                <DialogFooter>
                    <Button variant="ghost" onClick={onClose} disabled={loading} className="text-muted-foreground">Cancel</Button>
                    <Button
