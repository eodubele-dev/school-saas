...
export function CurriculumManagerContent({ milestones, onEdit }: CurriculumManagerContentProps) {
    if (milestones.length === 0) {
        return (
            <div className="bg-[#0A0A0B]/50 backdrop-blur-md border border-white/10 rounded-3xl p-12 text-center shadow-inner relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-t from-red-500/5 to-transparent pointer-events-none" />
                <p className="text-slate-500 font-medium relative z-10">No milestones assigned to this student yet.</p>
            </div>
        )
    }

    return (
        <div className="bg-[#0A0A0B]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                    ...
                    {milestones.map((item) => (
                        <div key={item.id} className="flex gap-4 group">
                            ...
                            <div className={`flex-1 p-5 rounded-2xl border transition-all duration-300 ${item.status === 'in-progress' ? 'bg-yellow-950/30 border-yellow-500/30 shadow-[0_0_30px_rgba(234,179,8,0.05)] translate-x-2' :
                                item.status === 'completed' ? 'bg-white/5 border-blue-500/20 hover:bg-blue-950/20 hover:border-blue-500/30 hover:translate-x-1' :
                                    'bg-transparent border-transparent hover:bg-white/5 hover:border-white/10 hover:translate-x-1'
                                }`}>
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-[10px] font-mono font-bold uppercase tracking-widest px-2.5 py-1 rounded-md ${item.status === 'in-progress' ? 'bg-yellow-500/10 text-yellow-300' : 'bg-white/5 text-slate-400'}`}>
                                        {item.subject} • {item.grade_level} • {item.week_range}
                                    </span>
                                    <button
                                        onClick={() => onEdit(item)}
                                        className="text-slate-500 hover:text-yellow-400 opacity-0 group-hover:opacity-100 transition-all p-2 rounded-lg hover:bg-yellow-500/10 active:scale-95"
                                    >
                                        <Edit2 size={16} />
                                    </button>
                                </div>
                                <h3 className={`font-black tracking-tight text-xl mt-2 ${item.status === 'locked' ? 'text-slate-600' : 'text-white drop-shadow-sm'}`}>
                                    {item.topic}
                                </h3>
                                ...
                                <div className="mt-5 flex items-center gap-4">
                                    ...
                                    {item.status === 'in-progress' && (
                                        <div className="flex-1 max-w-[250px] flex items-center gap-3 bg-black/40 p-2 rounded-xl border border-white/5">
