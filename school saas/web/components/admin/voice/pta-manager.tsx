...
export function PTAManager({ meetings = [] }: { meetings: any[] }) {
    ...
    if (meetings.length === 0) {
        return (
            <Card className="flex flex-col items-center justify-center p-12 text-center border-white/10 bg-black/40 backdrop-blur-xl relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black text-white italic uppercase tracking-tight relative z-10">No PTA Requests</h3>
                <p className="text-slate-400 mt-2 max-w-sm relative z-10">
    ...
    return (
        <div className="space-y-4">
            {meetings.map((meeting) => (
                <Card key={meeting.id} className="overflow-hidden border-white/10 bg-[#0A0A0B] backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-500 relative group">
                    ...
                    <div className="bg-white/5 px-6 py-4 border-b border-white/5 flex items-start justify-between relative z-10">
                        <div className="flex gap-4 items-center">
                            ...
                            <div>
                                <h4 className="font-bold text-white text-lg tracking-tight">{meeting.student?.full_name}</h4>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {meeting.status === 'scheduled' && (
                                <div className="flex gap-3">
                                    <Button
                                        variant="outline"
                                        className="text-emerald-400 hover:text-white bg-emerald-500/10 hover:bg-emerald-500 border-emerald-500/30 transition-all font-bold tracking-tight shadow-[0_0_15px_rgba(16,185,129,0.15)]"
                                    ...
                                    <Button
                                        variant="outline"
                                        className="text-red-400 hover:text-white bg-red-500/10 hover:bg-red-500 border-red-500/30 transition-all font-bold tracking-tight shadow-[0_0_15px_rgba(239,68,68,0.15)]"
                                        disabled={isLoading === meeting.id}
