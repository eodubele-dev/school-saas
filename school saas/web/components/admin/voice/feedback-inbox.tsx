    ...
    if (feedback.length === 0) {
        return (
            <Card className="flex flex-col items-center justify-center p-12 text-center border-white/10 bg-black/40 backdrop-blur-xl relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black text-white italic uppercase tracking-tight relative z-10">Inbox Empty</h3>
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {feedback.map((item, index) => (
                <Card
                    key={item.id}
                    className="overflow-hidden border-white/10 bg-[#0A0A0B] backdrop-blur-xl hover:-translate-y-1 hover:shadow-[0_10px_40px_rgba(16,185,129,0.15)] transition-all duration-300 animate-in fade-in slide-in-from-bottom-4 relative group"
                        ...
                        <div className="bg-white/5 p-5 border-b border-white/5 flex justify-between items-start">
                            <div className="flex gap-3">
                                <Avatar className="h-10 w-10 border-2 border-black ring-2 ring-emerald-500/40 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                                    <AvatarFallback className="bg-gradient-to-br from-emerald-400 to-teal-600 text-white font-bold">
                                        {item.user?.full_name?.charAt(0) || 'U'}
                                    </AvatarFallback>
                                </Avatar>
                                <div>
                                    <h4 className="font-bold text-white text-sm line-clamp-1 tracking-tight">{item.user?.full_name || 'Anonymous Parent'}</h4>
