...
-- Policies (Simplified)
DROP POLICY IF EXISTS "Tenant Access Inventory" ON inventory_items;
CREATE POLICY "Tenant Access Inventory" ON inventory_items
    USING (tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Tenant Access Categories" ON inventory_categories;
CREATE POLICY "Tenant Access Categories" ON inventory_categories
    USING (tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Tenant Access Vendors" ON inventory_vendors;
CREATE POLICY "Tenant Access Vendors" ON inventory_vendors
    USING (tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Tenant Access Requisitions" ON inventory_requisitions;
CREATE POLICY "Tenant Access Requisitions" ON inventory_requisitions
    USING (tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Tenant Access Requisition Items" ON inventory_requisition_items;
CREATE POLICY "Tenant Access Requisition Items" ON inventory_requisition_items
    USING (requisition_id IN (SELECT id FROM inventory_requisitions WHERE tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid())));

DROP POLICY IF EXISTS "Tenant Access Transactions" ON inventory_transactions;
CREATE POLICY "Tenant Access Transactions" ON inventory_transactions
    USING (tenant_id = (SELECT tenant_id FROM profiles WHERE id = auth.uid()));
